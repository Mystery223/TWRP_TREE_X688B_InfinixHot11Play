# Android device tree for Infinix Hot 11 Play X688B (Infinix-X688B)

## Device specifications (Infinix Hot 11 Play)

| Feature | Specification |
|--------|---------------|
| Network | GSM / HSPA / LTE |
| Launch | Announced: 2021, November 15 |
| Status | Available. Released 2021, November 15 |
| Dimensions | 171.8 x 78 x 8.9 mm (6.76 x 3.07 x 0.35 in) |
| Weight | - |
| Build | Glass front, plastic back, plastic frame |
| SIM | Nano-SIM + Nano-SIM |
| Display Type | IPS LCD, 440 nits (typ) |
| Display Size | 6.82 inches, 110.5 cm² (~82.4% screen-to-body ratio) |
| Resolution | 720 x 1640 pixels (~263 ppi density) |
| OS | Android 11, XOS 7.6 |
| Chipset | Mediatek MT6765G Helio G35 (12 nm) |
| CPU | Octa-core (4x2.3 GHz Cortex-A53 & 4x1.8 GHz Cortex-A53) |
| GPU | PowerVR GE8320 |
| Card Slot | microSDXC (dedicated slot) |
| Internal Storage | 64GB 4GB RAM, 128GB 4GB RAM |
| Storage Type | eMMC 5.1 |
| Main Camera | Dual: 13 MP, f/1.8 (wide), 1/3.1", 1.12µm, AF + Auxiliary lens |
| Camera Features | Quad-LED flash, panorama, HDR |
| Main Camera Video | 1080p@30fps |
| Selfie Camera | 8 MP, f/2.0 (wide) |
| Selfie Features | LED flash |
| Selfie Video | 1080p@30fps |
| Loudspeaker | Yes |
| 3.5mm Jack | Yes |
| WLAN | Wi-Fi 802.11 a/b/g/n/ac, dual-band, Wi-Fi Direct |
| Bluetooth | 5.0, A2DP, LE |
| Positioning | GPS |
| NFC | No |
| Radio | FM radio |
| USB | microUSB 2.0 |
| Sensors | Fingerprint (rear-mounted), accelerometer, proximity, compass |
| Battery | 6000 mAh |
| Charging | 10W wired |
| Colors | Green, Gold, Blue, Black |

## Device picture

![Infinix NOTE 40 Pro](https://fdn2.gsmarena.com/vv/pics/infinix/infinix-hot11-play-1.jpg)



```
#
# Copyright (C) 2026 The Android Open Source Project
# Copyright (C) 2026 SebaUbuntu's TWRP device tree generator
#
# SPDX-License-Identifier: Apache-2.0
#
```
